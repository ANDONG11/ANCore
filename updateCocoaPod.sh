...
currentVersionString=`grep -E 's.version.*=' ANCore.podspec`
...
lineNumber=`grep -nE 's.version.*=' ANCore.podspec | cut -d : -f1`

sed -i "" "${lineNumber}s/${currentVersion}/${newVersion}/g" ANCore.podspec

echo "Current version: ${currentVersion}, New version: ${newVersion}"

#Push ANCore.podspec

git add ANCore.podspec
git commit -m ${newVersion}

git tag ${newVersion}
git push origin master --tags
pod trunk push ./ANCore.podspec --verbose --allow-warnings --use-libraries
