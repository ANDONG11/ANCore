...
#pragma mark - 通过UIApplication打开url
...
static miniAppSuccess miniAppSuccessCallback;
...
+ (miniAppSuccess)miniAppSuccessCallback {
    return miniAppSuccessCallback;
}

+ (void)setMiniAppSuccess:(miniAppSuccess)callback {
    miniAppSuccessCallback=callback;
}
        ...
        if (OAuthDic[@"error"]) {
            ...
            if (self.authSuccess) {
                ...
                self.authSuccess(@{@"accessToken": (dic[@"access_token"] ?: [NSNull null]),
                                   ...
                                   @"expiresIn": (dic[@"expires_in"] ?: [NSNull null])});
            }
        }
    }];
}
