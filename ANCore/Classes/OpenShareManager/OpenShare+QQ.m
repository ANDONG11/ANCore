...
+ (void)connectQQWithAppId:(NSString *)appId {
    [self set:scheme Keys:@{@"appid":appId,@"callback_name":[NSString stringWithFormat:@"QQ%08llX",[appId longLongValue]]}];
}
...
+ (void)QQAuth:(NSString*)scope success:(authSuccess)success fail:(authFail)fail {
    if ([self beginAuth:scheme Success:success Fail:fail]) {
        ...
        [self setGeneralPasteboard:[@"com.tencent.tencent" stringByAppendingString:[self keyFor:scheme][@"appid"]] Value:authData encoding:OSPboardEncodingKeyedArchiver];
        [self openURL:[NSString stringWithFormat:@"mqqOpensdkSSoLogin://SSoLogin/tencent%@/com.tencent.tencent%@?generalpastboard=1",[self keyFor:scheme][@"appid"],[self keyFor:scheme][@"appid"]]];
    }
}
...
+ (NSString*)genShareUrl:(OSMessage*)msg to:(int)shareTo {
    ...
    [ret appendString:[self keyFor:scheme][@"callback_name"]];
    ...
    if ([msg isEmpty:@[@"image",@"link" ] AndNotEmpty:@[@"title"]]) {
        ...
        [self setGeneralPasteboard:@"com.tencent.mqq.api.apiLargeData" Value:data encoding: OSPboardEncodingKeyedArchiver];
        [ret appendString:@"img&title="];
        [ret appendString:[self base64Encode:msg.title ? msg.title:@""]];
        [ret appendString:@"&objectlocation=pasteboard&description="];
        [ret appendString:[self base64Encode:msg.desc ? msg.desc:@""]];
    } else if ([msg isEmpty:@[] AndNotEmpty:@[@"title",@"link",@"multimediaType"]]) {
        /// 新闻／多媒体分享（图片加链接）发送新闻消息 预览图像数据，最大1M字节 URL地址,必填，最长512个字符 via QQApiInterfaceObject.h
        NSDictionary *data = nil;
        if (msg.image) {
            data=@{@"previewimagedata":[self dataWithImage:msg.image]};
        }
        [self setGeneralPasteboard:@"com.tencent.mqq.api.apiLargeData" Value:data encoding: OSPboardEncodingKeyedArchiver];
        ...
        [ret appendFormat:@"%@&title=%@&url=%@&description=%@&objectlocation=pasteboard",msgType,[self base64AndUrlEncode:msg.title ? msg.title:@""],[self base64AndUrlEncode:msg.link],[self base64AndUrlEncode:msg.desc ? msg.desc:@""]];
    }
    return ret;
}
...
+ (void)chatWithQQNumber:(NSString*)qqNumber {
    [self openURL:[NSString stringWithFormat:@"mqqwpa://im/chat?uin=%@&thirdAppDisplayName=%@&callback_name=%@&src_type=app&version=1&chat_type=wpa&callback_type=scheme",qqNumber,[self base64Encode:[self CFBundleDisplayName]],[self keyFor:scheme][@"callback_name"]]];
}

+ (void)chatInQQGroup:(NSString*)groupNumber {
    [self openURL:[NSString stringWithFormat:@"mqqwpa://im/chat?uin=%@&thirdAppDisplayName=%@&callback_name=%@&src_type=app&version=1&chat_type=group&callback_type=scheme",groupNumber,[self base64Encode:[self CFBundleDisplayName]],[self keyFor:scheme][@"callback_name"]]];
}
...
+ (BOOL)QQ_handleOpenURL {
    ...
    if ([url.scheme hasPrefix:@"QQ"]) {
        ...
        //登陆auth
        NSDictionary *ret=[self generalPasteboardData:[@"com.tencent.tencent" stringByAppendingString:[self keyFor:scheme][@"appid"]] encoding:OSPboardEncodingKeyedArchiver];
