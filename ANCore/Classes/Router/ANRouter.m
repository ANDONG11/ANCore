...
static NSString * const AN_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const ANRouterParameterURL = @"ANRouterParameterURL";
NSString *const ANRouterParameterCompletion = @"ANRouterParameterCompletion";
NSString *const ANRouterParameterUserInfo = @"ANRouterParameterUserInfo";
...
@implementation ANRouter

+ (instancetype)sharedInstance {
    
    static ANRouter *instance = nil;
...
#pragma mark - 注册
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(ANRouterHandler)handler {
    
    [[self sharedInstance] addURLPattern:URLPattern andHandler:handler];
}

+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(ANRouterObjectHandler)handler {
...
- (void)addURLPattern:(NSString *)URLPattern andHandler:(ANRouterHandler)handler {
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(ANRouterObjectHandler)handler {
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
    }
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion {
    ...
    if (parameters) {
        ANRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[ANRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[ANRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
+ (id)objectForURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo {
    ANRouter *router = [ANRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    ANRouterObjectHandler handler = parameters[@"block"];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly {
    ...
    parameters[ANRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:AN_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL {
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:AN_ROUTER_WILDCARD_CHARACTER];
        }
    }
